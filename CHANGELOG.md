...
### Added
- **`discord.move_channel` RPC verb** — panel `/otter/move-channel` mirror of the `/movechannel` slash command. Params `{channelId, categoryId, position: 'top'|'bottom'}`. Refuses non-category target with `not-a-category`. Verifies the bot has `ManageChannels` on the source channel before calling `setParent`. For `position='top'`, sorts existing siblings by `rawPosition` and sets the moved channel to the first sibling's position; `bottom` lets Discord default. Returns `{channelId, channelName, fromCategoryName, toCategoryName, position}` for the audit row.
