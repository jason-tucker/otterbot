#!/usr/bin/env bash
# Otterbot — VPS install/update script
#
# Usage (after cloning the repo):
#   ./scripts/install.sh
#
# Or one-shot (replace OWNER):
#   GITHUB_OWNER=OWNER bash <(curl -fsSL https://raw.githubusercontent.com/OWNER/otterbot/main/scripts/install.sh)
...
EOF
    ...
    # Generate a strong Postgres password if missing or default.
    ...
    if grep -qE '^POSTGRES_PASSWORD=(change_me|otterbot_dev|)?$' .env; then
    ...
    echo "    DISCORD_BOT_TOKEN, DISCORD_CLIENT_ID"
    echo "    EUPHORIC_API_KEY"
    echo "    SUDO_ROLE_IDS"
    echo ""
...
if docker compose ps "$BOT_NAME" 2>/dev/null | grep -qE "running|Up"; then
    ...
    # Install CLI helper to /usr/local/bin
    if [ -f "scripts/$BOT_NAME" ] && [ ! -x "/usr/local/bin/$BOT_NAME" ]; then
        ...
        if sudo cp "scripts/$BOT_NAME" "/usr/local/bin/$BOT_NAME" 2>/dev/null && \
           sudo chmod +x "/usr/local/bin/$BOT_NAME" 2>/dev/null; then
            ...
            warn "Could not install /usr/local/bin/$BOT_NAME"
